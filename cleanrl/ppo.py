...
class Args:
    ...
    track_fml: bool = False
    """if toggled, this experiment will be tracked with FasttrackML"""
    fml_experiment_name: str = "cleanRL"
    """the name of the experiment in FasttrackML"""
    fml_url: str = "http://localhost:5000"
    """the url for fasttrackml"""
...
def log_scalar(writer, name, value, step):
    """
    Log a scalar value to both TensorBoard and MLflow.

    Args:
        writer: The TensorBoard writer object.
        name (str): The name of the metric.
        value (float): The value of the metric.
        step (int): The current global step for tracking.
    """
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        SCALAR_LOGS = [
            ("charts/learning_rate", lambda: optimizer.param_groups[0]["lr"]),
            ("losses/value_loss", lambda: v_loss.item()),
            ("losses/policy_loss", lambda: pg_loss.item()),
            ("losses/entropy", lambda: entropy_loss.item()),
            ("losses/old_approx_kl", lambda: old_approx_kl.item()),
            ("losses/approx_kl", lambda: approx_kl.item()),
            ("losses/clipfrac", lambda: np.mean(clipfracs)),
            ("losses/explained_variance", lambda: explained_var),
            (
                "charts/SPS",
                lambda: int(global_step / (time.time() - start_time)),
            ),
        ]
